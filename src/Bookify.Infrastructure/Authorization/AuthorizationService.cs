...
internal sealed class AuthorizationService
{
    ...
    {
        var cacheKey = $"auth:role-{identityId}";

        var cachedRoles = await _cacheService.GetAsync<UserRolesResponse>(cacheKey, CancellationToken.None);
    ...
    public async Task<HashSet<string>> GetPermissionsForUserAsync(string identityId)
    {
        ...
        var cachedPermissions = await _cacheService.GetAsync<HashSet<string>>(cacheKey, CancellationToken.None);
